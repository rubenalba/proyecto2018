...
public class ConfiguracionController {
	...
	@FXML
	private PasswordField contraseÒaTF;
	...
	public void actualizarPassword (ActionEvent event) {
		...
		if (contraseÒaTF.getText().equals(ConfirmarPWDTF.getText()) && contraseÒaTF.getLength() > 6) {
			...
			try {
				...
				contraseÒaTF.setText("");
				ConfirmarPWDTF.setText("");
				Alert alert = new Alert(AlertType.INFORMATION);
				alert.setHeaderText("Contrase√±a cambiada");
				alert.showAndWait();
			} catch (Exception e) {
			}
		}else {
			Alert alert = new Alert(AlertType.ERROR);
			alert.setHeaderText("Las contrase√±as no coinciden o no cumple con el m√≠nimo de 6 car√°cteres");
			alert.showAndWait();
		}
	}
